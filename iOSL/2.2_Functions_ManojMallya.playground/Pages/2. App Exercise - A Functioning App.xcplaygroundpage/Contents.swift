...
func incrementSteps(step: Int){
    steps=steps+1
    print(steps)
}

var steps = 0
incrementSteps(step: steps)
incrementSteps(step: steps)
...
func progressUpdate(target: Int)
{
    if target < goal/10{
        print("You're off to a good start")
    }
    else if target < goal/2{
        print("You're almost halfway there!")
    }
    else if target < (goal*9)/10{
