...
Select pno, Project.dno, lname, address, bdate 
from Project, Employee, Department 
where plocation='Stafford' and Project.dno=Department.dno and Department.mgrssn=Employee.ssn;
...
Select T.lname as supervisor_lname, T.fname as supervisor_fname, S.lname as employee_lname, S.fname as employee_fname 
from employee T,employee S 
where T.ssn=S.superssn;
...
where dno in (select dno 
              from Department 
              where mgrssn in (select ssn from Employee where lname='Smith')) or pno in (select pno from works_on where essn in (select ssn from Employee where lname='Smith'));
...
where employee.dno=5 and salary between 30000 and 40000 and employee.dno=department.dno;
...
Select fname, lname, employee.dno, pname, project.pno 
from employee, project, works_on 
where works_on.pno=project.pno and works_on.essn=employee.ssn order by employee.dno, fname, lname;
...
where Employee.ssn = Dependent.essn and Employee.sex = Dependent.sex and Employee.fname = Dependent.dependent_name;
...
Select sum(salary), max(salary), min(salary), avg(salary) from employee natural join department where dname='Research';


-- 17.For each project, retrieve the project number, the project name, and the number of employees who work on that project.
select Project.pno, pname, count (*) 
from Project, Works_on 
where Project.pno = Works_on.pno 
group by project.pno, pname order by project.pno;
...
select dname, count(*) 
from Department, employee 
where Department.dno = employee.dno and salary >40000 and department.dno in (select dno from employee group by dno having count(*) > 5) 
group by dname;
