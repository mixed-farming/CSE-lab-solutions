...
14.) With dept_total(dept_name,value) as (Select dept_name,sum(salary) from instructor group by dept_name),dept_total_average(value) as (Select avg(value) from dept_total) Select dept_name from dept_total, dept_total_average >= dept_total_avg.value;
